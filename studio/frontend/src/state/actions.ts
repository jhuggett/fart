...
import { nudgeSel as nudgeScene, frameBounds as sceneBounds } from "../canvas/scene3.ts";
...
export function initCommands() {
	register(
		...
		{ id: "file.save", title: "Save (checkpoint)", group: "File", when: inAny, run: either(() => void save(), () => void M.save(), () => void S.save()) },
		...
		{ id: "file.newScene", title: "New scene…", group: "File", when: inProject, run: () => void ask("Name the new scene").then((n) => { if (n) void newScene(n, false); }) },
		...
		{ id: "edit.duplicate", title: "Duplicate", group: "Edit", when: inAny, run: either(dupSel, M.dupSel, () => { if (S.sc.sel.value) S.duplicateNode(S.sc.sel.value); }) },
		{ id: "model.mirror", title: "Mirror across x", group: "Edit", when: () => inModel() && !!M.md.sel.value, run: M.mirrorSel },
		{ id: "edit.delete", title: "Delete", group: "Edit", when: inAny, run: either(deleteSel, M.deleteSel, () => { if (S.sc.sel.value) S.deleteNode(S.sc.sel.value); }) },
